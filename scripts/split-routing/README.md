...
This script makes networking changes only to the current session. None of the changes are persisted, so will not survive a reboot. If in any doubt, just reboot and you're back where you were.

The `set-routes.sh` script will determine the default gateway for the network, using data provided by [Little Snitch](https://www.obdev.at/products/littlesnitch/index.html) traffic log to find what connections have been made by the app. It will then update the route table on Mac OS to route those connections via the default gateway.

### Note

This script updates route tables, but depending on how the app behaves, it may remain connected through its existing routing. Disconnecting / reconnecting the VPN or network interface may be necessary to force the app to re-establish a connection, which will then use the new route.
...
This will reset the networking route tables to default, and remove any customisation.
