...
def main():
    ...
    with zipfile.ZipFile(INPUT_ZIP_NAME, 'r') as input_zip_file:
        ...
        print('Filtering output to max of', ENTRIES_PER_COUNTRY,
              'entries from keys', DESIRED_COUNTRIES)
        filtered_file_list = select_random_entries(
            country_dict, DESIRED_COUNTRIES, ENTRIES_PER_COUNTRY)

        # Pull selected config from source, and write into new, generated, timestamped zip
        output_zip_file = datetime.datetime.now().strftime(
            OUTPUT_ZIP_PREFIX + '_%y%m%d_%H%M%S.zip')
        print('Writing filtered contents to', output_zip_file,
              'with config from ', filtered_file_list)
        create_filtered_zip(
            input_zip_file, filtered_file_list, output_zip_file)
